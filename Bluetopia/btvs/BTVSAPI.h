...
BTPSAPI_DECLARATION int BTPSAPI VS_EnableWBS(unsigned int BluetoothStackID, Word_t ConnectionHandle);
   ...
   typedef int (BTPSAPI *PFN_VS_EnableWBS_t)(unsigned int BluetoothStackID, Word_t ConnectionHandle);
	...
	/* Enable the WBS without configure the CC256xx PCM interface, the  */
	/* CODEC setting Send Write_Voice_Setting and WBS associate.        */
...
/* Configure the CC256x PCM interface to be the slave on the I2S lines  */
/* for Audio on the STM3240G-EVAL board. 								*/
